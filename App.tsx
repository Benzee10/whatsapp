...
const ShareGate = ({ shareCount, onShareClick }: { shareCount: number, onShareClick: () => void }) => {
    ...
    return (
        <div className="absolute inset-0 bg-black flex flex-col items-center justify-center p-6 text-center z-30 safe-area-inset">
            ...
            <p className="text-white mb-8 text-xl sm:text-lg leading-relaxed">Share with 5 groups on Snapchat to unlock the private feed.</p>
            
            <div className="w-full max-w-sm bg-gray-800 rounded-full h-10 mb-3 overflow-hidden border border-gray-600">
                <div 
                    className="bg-[#FFFC00] h-full flex items-center justify-center text-black font-bold transition-all duration-500"
            ...
            <button
                ...
                className="w-full max-w-sm bg-[#FFFC00] text-black font-bold py-5 px-6 rounded-full text-xl flex items-center justify-center gap-3 active:bg-yellow-400 transition-colors touch-manipulation"
            >
...
const App: React.FC = () => {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-black" onClick={handleAnyClick}>
      <div className="w-full h-screen sm:w-full sm:max-w-md sm:h-[95vh] sm:max-h-[900px] flex flex-col bg-gray-900 sm:shadow-2xl sm:rounded-2xl overflow-hidden relative" onClick={handleAnyClick}>
        {!showShareGate && (
            <>
